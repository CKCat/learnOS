...
_getvbemode:
        ...
        mov es,ax
        mov ax,0x4f00
        int 0x10				; 调用 BIOS 10h 中断可以获取 VBE 相关的数据结构
        cmp ax,0x004f
        jz .ok
...
_getvbeonemodeinfo:
        ...
        mov ax,0
        mov es,ax
        mov cx,0x118			; 118h 模式下屏幕分辨率为 1024x768，显存大小是 16.8MB，显存开始地址一般为 0xe0000000。
        mov ax,0x4f01
        int 0x10
        cmp ax,0x004f
        jz .ok
...
int131errmsg: 
		db     "int 13 read hdsk  error"
        db 0
getvbmodeerrmsg: 
		db     "get vbemode err"
        db 0
getvbmodinfoeerrmsg: 
		db     "get vbemodeinfo err"
        db 0
setvbmodeerrmsg: 
		db     "set vbemode err"
        db 0
