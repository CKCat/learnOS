...
mrsdp_t *findacpi_rsdp_core(void *findstart, u32_t findlen) {
  ...
  for (u64_t i = 0; i <= findlen; i++) {
    if (('R' == tmpdp[i]) && ('S' == tmpdp[i + 1]) && ('D' == tmpdp[i + 2]) &&
        (' ' == tmpdp[i + 3]) && ('P' == tmpdp[i + 4]) &&
        ('T' == tmpdp[i + 5]) && ('R' == tmpdp[i + 6]) &&
        (' ' == tmpdp[i + 7])) {
...
void init_mem(machbstart_t *mbsp) {
  ...
  if (chk_memsize(retemp, retemnr, 0x100000 /* 1MB */, 0x8000000 /* 128MB */) == NULL) {
    kerror(
...
void init_meme820(machbstart_t *mbsp) {
  ...
  if (1 >
      move_krlimg(mbsp, (u64_t)((u32_t)demp), (senr * (sizeof(e820map_t))))) {
    kerror("move_krlimg err");
  }
  ...
  mbsp->mb_nextwtpadr =
      P4K_ALIGN((u32_t)(demp) + (u32_t)(senr * (sizeof(e820map_t))));
...
void init_chkmm() {
  ...
  {
    kprint(
        "Your computer is low on memory, the memory cannot be less than 64MB!");
    CLI_HALT();
  }
...
void init_bstartpagesold(machbstart_t *mbsp) {
  ...
  pt64_t *pml4p = (pt64_t *)PML4T_BADR, *pdptp = (pt64_t *)PDPTE_BADR,
         *pdep = (pt64_t *)PDE_BADR;
